#ifndef EDUCATIVEMODERNCPPCONCUR_PACKAGEDTASK_H
#define EDUCATIVEMODERNCPPCONCUR_PACKAGEDTASK_H
#include <utility>
#include <future>
#include <iostream>
#include <thread>
...
namespace PackagedTask
{
    ...
    void Run()
    {
        ...
        while (not allTasks.empty())
        {
            ...
            allTasks.pop_front();
            std::thread sumThread(std::move(myTask), begin, end);//invoke a packaged_task
            begin = end;
            end += increment;
            sumThread.detach();
        }
...
#endif //EDUCATIVEMODERNCPPCONCUR_PACKAGEDTASK_H
